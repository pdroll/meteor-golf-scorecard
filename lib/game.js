...
Game.CreateNew = function(users, holes, course){
...
Game.FinishGame = function(game){
	...
	var updateGameAndRedirect = function() {
		console.log('updateGameAndRedirect');
	...
	var existingCourse = Course.findOne({name : game.courseName});

	if(existingCourse){
		existingCourse.holes = holes;
		Course.SaveCourse(existingCourse, updateGameAndRedirect);
	} else {
		var newCourse = Course.CreateNew(game.course, holes);
		Course.SaveCourse(newCourse, updateGameAndRedirect);
	}
};
...
Game.FinishGamePrompt = function(e, template){
	...
	if(confirm('Are you sure you want to finish this game? You will not be able to edit any scores after you do this.')){
		...
		template.firstNode.classList.add('is-loading');
